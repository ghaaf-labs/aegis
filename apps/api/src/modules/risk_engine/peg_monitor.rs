...
use crate::db::Db;
use crate::modules::rebalance::defensive_plan::{propose_defensive_plan, DefensivePlanRequest};
    ...
    let rebalance_id = match action_taken.as_str() {
        "propose_rebalance" | "auto_execute" => {
            let req = DefensivePlanRequest {
                ...
                target_asset: rule.target_asset.clone(),
            };
            propose_defensive_plan(state, &req).await.unwrap_or(None)
        }
        _ => None,
    };
