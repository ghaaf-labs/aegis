...
pub fn evaluate(allocations: &[Allocation], prices: &[AssetPrice]) -> RiskReport {
    ...
    RiskReport { score, concentration_risk: concentration, volatility_score: volatility, drift_score: drift, summary }
}

fn concentration_risk(allocations: &[Allocation]) -> f64 {
    if allocations.is_empty() { return 0.0; }
    let max_weight = allocations.iter().map(|a| a.current_weight).fold(0f64, f64::max);
    (max_weight / 100.0).min(1.0)
}

fn volatility_score(prices: &[AssetPrice]) -> f64 {
    if prices.is_empty() { return 0.5; }
    let avg_volatility: f64 = prices.iter()
        .map(|p| p.change_24h.abs())
...
fn drift_score(allocations: &[Allocation]) -> f64 {
    if allocations.is_empty() { return 0.0; }
    let total_drift: f64 = allocations.iter()
