...
pub async fn analyze_portfolio(
    ...
    //
    // Constitution short-circuit: when CONSTITUTION_ENABLED, the strategist's
    // proposal is run through the versioned YAML rulebook (max drawdown,
    // single-asset cap, slippage ceiling, EURC band, USYC floor) BEFORE the
    // LLM critic. Any violation triggers an immediate VETO whose reasoning
    // cites the clause IDs — no LLM call needed, no token spend. Closes the
    // "prompt-injection bypasses critic" attack class.
    let constitution_violations = if state.config.constitution_enabled {
    ...
    let verdict = if !constitution_violations.is_empty() {
        ...
        // Free-form critic never emits constitutional clause IDs — clear any
        // accidental hallucination from the model.
        v.clause_ids.clear();
