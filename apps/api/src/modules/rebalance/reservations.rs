//! Reservation accounting (INV-8): derive the pre-existing USDC locked by
//! executing rebalance legs so a fresh plan sizes only spendable Gateway cash.
//!
//! The durable concurrency boundary is the `rebalances` table: creating a fresh
//! review supersedes older planned reviews for the same portfolio, and approval
//! atomically admits only one executing rebalance. This module stays as pure leg
//! accounting and deliberately avoids an unused in-memory ledger.
