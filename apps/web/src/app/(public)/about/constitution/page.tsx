    ...
    <main className="mx-auto max-w-3xl px-6 py-12 text-white">
      <header className="mb-8">
        ...
        <p className="text-gray-300 text-sm leading-relaxed mb-4 max-w-2xl">
          ...
          the clause ID — no model can override it. This is the auditable
          rulebook behind every block.
        </p>
        <div className="flex gap-4 font-mono text-[11px] text-gray-400">
          <span>
            version <span className="text-cyan-300">v{doc.version}</span>
          </span>
          <span>
            effective{" "}
          ...
          <span>
            clauses <span className="text-cyan-300">{doc.clauses.length}</span>
          </span>
        </div>
      </header>
      ...
      <footer className="mt-12 border-t border-white/10 pt-4 text-[11px] font-mono text-gray-500">
        Versioning rule: removing or weakening a clause is a major version bump.
        The current version is recorded on every persisted decision so future
        audits can re-evaluate historical proposals against the constitution
